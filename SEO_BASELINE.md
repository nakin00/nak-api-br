...
| 2026-08-25 | nao medido | nao medido | nao medido | nao medido | **Publicacao nova: `/observacao/slack-demandas`** — primeira pagina fora de `/cases`. Sistema de demandas no Slack no ar desde 21/08, sem numero de impacto, entao o texto foi pra uma prateleira nova ("Em observacao") em vez de virar case: mantem a regra do site (Principio 1 do ANTI_IA) e ainda assim publica, com o status declarado no proprio texto, no card do indice e no `llms.txt`. Sitemap regenerado: **17 URLs** (era 16), a nova entra com prioridade 0.6 e `changefreq weekly`. Auditoria tecnica (`auditoria-seo-geo`): **0 erros, 34 avisos**, todos de julgamento — dos avisos da pagina nova, os emojis sao icone de fluxograma e do painel real do Slack. **O script de auditoria nao enxergava a prateleira nova** (so varria `*.html` e `cases/*.html`): corrigido pra varrer `observacao/*.html` tambem, tratado com a mesma regra de case. Achado de quebra: o `gerar_sitemap.mjs` apontava pra `cases/index.html`, arquivo que nao existe — ele estourava antes de escrever o sitemap, e por isso as regeneracoes recentes eram manuais. **GEO:** a pagina nasce com 2 headings em forma de pergunta (o site inteiro tinha 1) e uma definicao fechada de "demanda", que e o formato que modelo recorta pra citar. Metricas do GSC nao medidas nesta linha — 1 dia depois do check anterior, sem janela nova pra ler. |
